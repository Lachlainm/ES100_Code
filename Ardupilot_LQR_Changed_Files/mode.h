...
public:
    ...
    VectorN<float, 12>   getStateAHRS(); // get the current state variables and return them as the 12d vector
    VectorN<float, 6>    LQI(void); // compute LQI control from K_HL and error
    void                 allocate_control(bool useFF);
    void                 updateDesiredState();
    void                 gravityFF(VectorN<float, 6>& u); // add in feed-forward gravity control to LQI output
    VectorN<float, 12>   bodyForce2propForces(const VectorN<float, 6>& u); // convert desired body forces to indv prop forces
    Vector3f             allocateControlLL(const Vector3f& propForce); // allocate servo + motor commands from prop forces

    void                 FF_Force(VectorN<float, 6>& u);
