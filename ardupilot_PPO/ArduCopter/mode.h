...
public:
    ...
    VectorN<float, rows> matrixVectorMultiply(const VectorN<VectorN<float, rows>, cols>& matrix, 
                                            const VectorN<float, cols>& vector) {
        VectorN<float, rows> result;
    ...
    // LQI Gain
    VectorN<VectorN<float, 6>, 18> K_HL_ALL;
    // augmented state space
    VectorN<VectorN<float, 6>, 12> B_t; // pseudo inverse of B from ss

    VectorN<VectorN<float, 6>, 12> W;
    VectorN<VectorN<float, 12>, 6> W_t;
...
public:
    ...
    VectorN<float, 12> desiredState;
    VectorN<float, 12> currState;
    VectorN<float, 18> error;
    VectorN<float, 8>  tilt_angles; 
    VectorN<float, 8> raw_tilt_angles;
    // static float output_0[256];
    // static float output_1[256];
    // static float output_2[12];
    // static const float layer_0_weight[256][26];
    // static const float layer_1_weight[256][256];
    // static const float layer_2_weight[12][256];
    // static const float layer_0_bias[256];
    // static const float layer_1_bias[256];
    // static const float layer_2_bias[12];

    VectorN<float, 26>   getStateAHRS(); // get the current state variables and return them as the 12d vector
    void                 allocate_control(bool useFF);
    void                 updateDesiredState();
    VectorN<float, 12>   networkEvaluate(VectorN<float, 26>& state_array);
