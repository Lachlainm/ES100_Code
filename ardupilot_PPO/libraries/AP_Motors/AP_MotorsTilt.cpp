...
uint16_t AP_MotorsTilt::omega2pwm(double normalized_omega) {
    float pwm = std::round(normalized_omega * 1000 + 1000);
    ::printf("pwm motor: %f\n", pwm);
    if (pwm > 2000) pwm = 2000;
    if (pwm < 1000) pwm = 1000;
    return static_cast<uint16_t>(pwm);
}
int16_t AP_MotorsTilt::angle2pwmProx(double normalized_angle) {
    float pwm = std::round(normalized_angle * 700 + 1500);
    ::printf("pwm prox: %f\n", pwm);
...
int16_t AP_MotorsTilt::angle2pwmDist(double normalized_angle) {
    float pwm = std::round(-normalized_angle * 700 + 1500);
    ::printf("pwm dist: %f\n", pwm);
    if (pwm > 2200) pwm = 2200;
    if (pwm < 800) pwm = 800;
...
void AP_MotorsTilt::output_to_motors()
{
    ...
    rc_write(AP_MOTORS_MOT_2, omega2pwm(_actuator[1]));
    rc_write(AP_MOTORS_MOT_3, omega2pwm(_actuator[2]));
    rc_write(AP_MOTORS_MOT_4, omega2pwm(_actuator[3]));

    rc_write(AP_MOTORS_MOT_5, angle2pwmProx(_actuator[4]));
    rc_write(AP_MOTORS_MOT_6, angle2pwmProx(_actuator[5]));
    rc_write(AP_MOTORS_MOT_7, angle2pwmProx(_actuator[6]));
    rc_write(AP_MOTORS_MOT_8, angle2pwmProx(_actuator[7]));

    rc_write(AP_MOTORS_MOT_9,  angle2pwmDist(_actuator[8]));
    rc_write(AP_MOTORS_MOT_10, angle2pwmDist(_actuator[9]));
    rc_write(AP_MOTORS_MOT_11, angle2pwmDist(_actuator[10]));
