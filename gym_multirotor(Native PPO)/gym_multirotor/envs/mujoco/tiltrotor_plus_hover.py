...
class TiltrotorPlus8DofHoverEnv(QuadrotorPlusHoverEnv):
    ...
    * env_name: TiltrotorPlus8DofHoverEnv-v0
    ...
    obs_tilt_index = np.arange(18, 26)
    action_index_tilt = np.arange(4, 12)

    def __init__(self, xml_name="tiltrotor_plus_hover.xml", frame_skip=5, env_bounding_box=3, randomize_reset=False):
    ...
    def _get_obs(self):
        ...
        Returns:
             numpy.ndarray: Observation vector of shape (22,). The elements of the observation tuple include (xyz, 
             rotation_matrix, linear_velocity, angular_velocity, tilt_angles).
        """
    ...
    def get_motor_input(self, a):
        ...
        action_range = 2.0
        avg_actuation = np.array([self.mass * 9.81 * 0.35,
                                  self.mass * 9.81 * 0.35,
                                  self.mass * 9.81 * 0.35,
                                  self.mass * 9.81 * 0.35,
                                  0.0,
                                  ...
                                  0.0,
                                  0.0,
                                  0.0])     # 4 propellers and 8 tilt-servos
    ...
    def get_reward(self, ob, a):
        ...
        """
        ob_quad, ob_tilt = ob[:18].copy(), ob[18:26]
        action_quad, action_tilt = a[0:4], a[4:12]
    ...
    def initialize_robot(self, randomize=True):
        ...
        tilt_init = np.random.uniform(low=-self.init_max_attitude, high=self.init_max_attitude, size=(8,))
