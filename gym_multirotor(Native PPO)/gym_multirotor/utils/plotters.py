...
def plot_action_tiltrotor(time, action, set_super_title=True, save_as=('thrust.pdf', 'tilt.pdf'), lineWidth=0.5,
    ...
    ylabels = ['$rotor_{front}$', '$rotor_{left}$', '$rotor_{back}$', '$rotor_{right}$',
               '$\\theta^{tilt}_{front}_{base} (^{\circ})$', '$\\theta^{tilt}_{left}_{base} (^{\circ})$',
               '$\\theta^{tilt}_{back}_{base} (^{\circ})$', '$\\theta^{tilt}_{right}_{base} (^{\circ})$',
               '$\\theta^{tilt}_{front}_{motor} (^{\circ})$', '$\\theta^{tilt}_{left}_{motor} (^{\circ})$',
               '$\\theta^{tilt}_{back}_{motor} (^{\circ})$', '$\\theta^{tilt}_{right}_{motor} (^{\circ})$']
